...
export default function PostContent() {
  ...
  const [caption, setCaption] = useState(
    captionPrefill ||
      (productName
        ? `Check out our latest ${productName}! 🚀\n\n#marketing #product #evokecmo`
        : ""),
  );
  ...
  const generateContent = async () => {
    if (!topic.trim()) {
      setGenError("Enter a topic or brief first.");
      return;
    }
    setGenError("");
    setGenerating(true);
    try {
      const platform =
        selectedPlatforms[0] || connectedPlatforms[0] || "linkedin";
      const platformLabel =
  ...
  const generateImage = () => {
    if (!topic.trim()) {
      setImageError("Enter a topic first so we know what to illustrate.");
      return;
    }
    setImageError("");
    ...
    const prompt = encodeURIComponent(
      `${topic.trim()}, professional product marketing photo, high quality, vibrant, studio lighting`,
    );
  ...
  const handlePost = async () => {
    ...
    if (selectedPlatforms.includes("gmail") && !emailRecipients.trim()) {
      setError("Enter at least one recipient email for the Gmail invitation.");
      return;
    }
    ...
    if (
      ...
      accounts.linkedin?.connectedAt
    ) {
      const ageMs = Date.now() - accounts.linkedin.connectedAt;
      if (ageMs > 55 * 24 * 60 * 60 * 1000) {
        setError(
          "Your LinkedIn connection may have expired (tokens last 60 days). Please go to Connect Accounts and reconnect LinkedIn before posting.",
        );
        return;
      }
    }
    ...
    try {
      ...
      const payload = {
        ...
        source: "post-content",
        emailRecipients: selectedPlatforms.includes("gmail")
      ...
      if (!res.ok) {
        ...
        try {
          ...
          detail =
            body?.message || body?.error || JSON.stringify(body) || detail;
        } catch {}
      ...
      if (evokeUser) {
        ...
        saveContentItems(
          ...
          console.warn("Failed to save to content library:", err),
        );
      }
    } catch (e) {
        ...
        >
          ...
          <div>
            ...
            <div
              style={{
                background:
                  "linear-gradient(135deg,rgba(200,151,62,0.08),rgba(200,151,62,0.04))",
              ...
              <div style={{ marginBottom: 20, position: "relative" }}>
                ...
                <img
                  ...
                  style={{
                    ...
                    objectFit: "cover",
                    maxHeight: 320,
                    display: generatingImage ? "none" : "block",
                  }}
                />
                <button
                  onClick={generateImage}
            ...
            <div>
              ...
              />
              <p style={{ fontSize: 11, color: TEXT3, marginTop: 4 }}>
          ...
          <div style={{ position: "sticky", top: 100 }}>
            ...
            >
              ...
              {loadingAccounts ? (
                ...
                <div style={{ textAlign: "center", padding: "16px 0" }}>
                  <p style={{ color: TEXT2, fontSize: 13, marginBottom: 12 }}>
                    No social accounts connected yet.
                  </p>
                  <button
                    onClick={() =>
                      navigate("/connect-accounts", {
                        state: { from: "/post-content" },
                      })
                    }
                    style={{
                      padding: "9px 18px",
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                    <button
                      onClick={() => setSelected(connectedPlatforms)}
                      style={{
                        flex: 1,
                        padding: "7px",
            ...
            {selectedPlatforms.includes("gmail") && (
              ...
              >
                ...
                >
                  Your caption becomes the email body, with the video link
            ...
            <button
              onClick={handlePost}
              disabled={
                posting || selectedPlatforms.length === 0 || !caption.trim()
              }
              style={{
                width: "100%",
                padding: "14px",
            ...
            >
              {posting ? (
                ...
                <>
                  ...
                  {selectedPlatforms.length > 0
                    ? ` to ${selectedPlatforms.length} platform${selectedPlatforms.length > 1 ? "s" : ""}`
                    : ""}
                </>
              )}
            </button>
