...
export function getRecommendedActions(connectedCount, campaigns, tokenBalance, kb, limit = Infinity) {
  ...
  // ── Always: Growth Strategy — the anchor agent every brand starts from ──
  pool.push({ score: 1000, icon: <TrendingUp size={18}/>, title: 'Growth Strategy Agent', desc: `Get a full GTM plan, revenue forecast and milestone roadmap tailored to your ${indLabel || 'business'} objectives.`, color: '#f97316', cta: 'Generate Strategy', path: '/campaign/growth_strategy', highlight: true, planRequired: 'free' })
  ...
  if (isB2B) {
    pool.push({ score: 85, icon: <Mail size={18}/>, title: 'Email Drip Sequence', desc: 'Nurture B2B leads with a 5-email automated sequence — subject lines, preheaders and CTAs ready to go.', color: '#8b5cf6', cta: 'Build Sequence', path: '/campaign/email_drip', highlight: true, planRequired: 'package-a' })
    pool.push({ score: 72, icon: <Globe size={18}/>, title: 'LinkedIn Content Strategy', desc: 'Build your LinkedIn presence with a 30-day content calendar tailored to your B2B audience.', color: '#0a66c2', cta: 'Start Strategy', path: '/campaign/content_calendar', planRequired: 'package-b' })
