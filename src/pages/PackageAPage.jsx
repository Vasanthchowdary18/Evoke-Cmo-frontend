...
/* ─── All agents ─── */
const ALL_AGENTS = [
  { id: 'strategy',           name: 'Strategy Agent',      sub: 'Define goals & GTM',                icon: <Target size={17}/>,       path: '/campaign/growth_strategy', tag: 'FREE',    color: '#c8973e' },
  { id: 'growth',             name: 'Growth Agent',         sub: 'Get clients & leads',               icon: <TrendingUp size={17}/>,   path: '/campaign/growth_agent',    tag: 'FREE',    color: '#c8973e', popular: true },
  { id: 'content',            name: 'Content Agent',        sub: 'Brand & content plan',              icon: <Layers size={17}/>,       path: '/campaign/content_calendar',tag: 'FREE',    color: '#c8973e' },
  ...
  { id: 'lifestyle',          name: 'Lifestyle Images',     sub: 'On-brand scene photography',        icon: <Camera size={17}/>,       path: '/image-lifestyle',          tag: 'PKG A',   color: '#ec4899' },
  ...
  { id: 'social',             name: 'Post to Social',       sub: 'Schedule & publish',                icon: <Share2 size={17}/>,       path: '/post-content',             tag: 'PKG A',   color: '#3b82f6' },
  { id: 'marketing-strategy', name: 'Marketing Strategy',  sub: 'Annual, quarterly & monthly plans', icon: <TrendingUp size={17}/>,   path: '/strategy',                 tag: 'PKG A',   color: '#c8973e' },
...
const PLATFORMS = [
  {
    ...
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
        <defs>
          <linearGradient id="igA" x1="0" y1="24" x2="24" y2="0">
...
export default function PackageAPage() {
  ...
  const [expandedPkgs, setExpandedPkgs] = useState(['a']); // Package A open by default
  ...
  const connectedCount = PLATFORMS.filter(
    (p) => p.alwaysOn || accounts[p.key]?.connected,
  ).length;
  ...
  const tagColor = (tag) => tag === "FREE" ? "#c8973e" : tag === "CMO" ? "#c8973e" : "#a855f7";
          ...
          >
            ...
            >
              <button
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = "rgba(200,151,62,0.22)";
                  e.currentTarget.style.borderColor = GOLD;
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "rgba(200,151,62,0.1)";
                  e.currentTarget.style.borderColor = "rgba(200,151,62,0.35)";
                }}
              >
                <svg
                  width="13"
            ...
            {sidebarOpen ? (
              ...
              <>
                ...
                {[
                  { key: 'free', label: 'Free Plan',  color: TEXT3,     agents: ALL_AGENTS.filter(a => a.tag === 'FREE') },
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {loadingAccts ? (
                ...
                PLATFORMS.map((platform) => {
                  const isConnected =
                    platform.alwaysOn || accounts[platform.key]?.connected;
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div
          style={{
            fontSize: 12,
            fontWeight: 700,
            color: selected ? "#fff" : "rgba(240,235,224,0.7)",
            whiteSpace: "nowrap",
            overflow: "hidden",
            textOverflow: "ellipsis",
          }}
...
function AgentDetail({ agent, onLaunch }) {
  const DESCRIPTIONS = {
    strategy: {
      full: "Build a full Go-To-Market strategy with clear, measurable goals aligned to your revenue targets. The Strategy Agent creates a complete plan covering positioning, target audience, key channels, and 90-day milestones.",
      steps: [
        "Define your target market & ICP",
        "Set revenue goals & KPIs",
        "Choose your GTM channels",
        "Get a 90-day action plan",
      ],
    },
    growth: {
      full: "Get a custom plan for attracting new clients and keeping existing ones. The Growth Agent covers outreach strategies, lead nurture flows, referral programs, and retention tactics tailored to your business.",
      steps: [
        "Prospect targeting strategy",
        "Outreach message sequences",
        "Lead nurture email flow",
        "Referral & retention plan",
      ],
    },
    content: {
      full: "A structured content blueprint covering brand voice, content pillars, post formats, and publishing cadence. The Content Agent creates a complete content calendar ready to execute across all platforms.",
      steps: [
    ...
    "image-angles": {
      full: "Transform a single product photo into a full set of professional multi-angle shots — front, side, back, detail — ready for e-commerce, ads, and catalogues. Powered by WaveSpeed AI video generation.",
      steps: [
        "Upload your product image URL",
        "Set angles and visual style",
        "AI generates 5-second video",
        "Download or post directly",
      ],
    },
    lifestyle: {
      full: "Place your product into realistic, on-brand lifestyle scenes — indoors, outdoors, or aspirational settings — without a costly photoshoot. Generate MP4 video showcasing your product in real-world context.",
      steps: [
        "Upload product image URL",
        "Choose lifestyle setting & mood",
        "AI generates lifestyle video",
        "Download or post to social",
      ],
    },
    banner: {
      full: "Generate platform-optimised static banners sized for Meta, Google, and display networks — pre-formatted with your brand colours, fonts, and copy. Coming soon.",
      steps: ["Coming soon"],
    },
    social: {
      full: "Connect your social media accounts, create content with the Content Agent, then launch your campaign directly to Instagram, Facebook, LinkedIn, Twitter and more.",
      steps: [
        "Connect your social accounts",
        "Generate content with AI agents",
        "Review content before posting",
        "Launch to all platforms at once",
      ],
    },
  };
  ...
  return (
    <div style={{ padding: "0" }}>
      ...
      <button
        ...
        style={{
          padding: "14px 32px",
          background: agent.soon
            ? "rgba(255,255,255,0.06)"
            : "linear-gradient(135deg,#d4a853,#b8803a)",
          border: agent.soon ? `1px solid ${BORDER}` : "none",
          borderRadius: 12,
          color: agent.soon ? TEXT3 : "#0e0c09",
          fontSize: 15,
          fontWeight: 800,
          cursor: agent.soon ? "not-allowed" : "pointer",
          display: "inline-flex",
          alignItems: "center",
          gap: 8,
        }}
