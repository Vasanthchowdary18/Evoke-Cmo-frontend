...
const PLATFORMS = [
  {
    ...
    color: "#1877f2",
    gradient:
      "linear-gradient(135deg, rgba(24,119,242,0.15), rgba(24,119,242,0.05))",
  ...
  },
  {
    ...
    color: "#e1306c",
    gradient:
      "linear-gradient(135deg, rgba(225,48,108,0.15), rgba(225,48,108,0.05))",
    border: "rgba(225,48,108,0.3)",
    description:
      "Connect your Instagram Business account via your Facebook Page",
  ...
  },
  {
    ...
    color: "#0a66c2",
    gradient:
      "linear-gradient(135deg, rgba(10,102,194,0.15), rgba(10,102,194,0.05))",
  ...
  },
  {
    ...
    color: "#ff0050",
    gradient:
      "linear-gradient(135deg, rgba(255,0,80,0.15), rgba(0,242,234,0.05))",
  ...
  },
  {
    ...
    color: "#0866ff",
    gradient:
      "linear-gradient(135deg, rgba(8,102,255,0.15), rgba(8,102,255,0.05))",
    ...
    fields: [
      {
        ...
        help: "Meta Ads Manager → Account Overview → Account ID (prefix with act_)",
      },
    ],
  },
  {
...
export default function ConnectAccounts() {
  ...
  const handleFacebookCallback = useCallback(async (code) => {
    ...
    try {
      ...
      if (data.error) throw new Error(data.message || 'Facebook auth failed');
      const { pageId, pageAccessToken, pageName, instagramBusinessAccountId } = data;
      if (!pageAccessToken) throw new Error('No page access token returned');
      await saveSocialAccount(uid, 'facebook', { pageId, pageAccessToken, pageName, connected: true });
      setAccounts(a => ({ ...a, facebook: { connected: true, pageId, pageName } }));
      setOk('facebook', true);
      // Auto-connect Instagram if the page has a linked Instagram Business account
      if (instagramBusinessAccountId) {
        ...
        setAccounts(a => ({ ...a, instagram: { connected: true, businessAccountId: instagramBusinessAccountId, pageName } }));
  ...
  // ── Instagram via saved Facebook credentials ─────────────────────────────
  const connectInstagram = () => {
    if (!user) return;
    setLoad("instagram", true);
    clrErr("instagram");

    // Reuse the already-saved Facebook page credentials — no popup needed
    const fb = accounts.facebook;
    if (!fb?.connected || !fb?.pageAccessToken || !fb?.pageId) {
      setErr("instagram", "Please connect your Facebook account first, then try connecting Instagram.");
    ...
    loadFBSDK().then(() => {
      window.FB.api(
        `/${fb.pageId}`,
        { fields: "instagram_business_account", access_token: fb.pageAccessToken },
        (igResp) => {
          if (igResp?.instagram_business_account?.id) {
            ...
            saveSocialAccount(user.uid, "instagram", {
              ...
              .then(() => {
                setAccounts((a) => ({
                  ...a,
                  instagram: { connected: true, businessAccountId: igId, pageName: fb.pageName },
                }));
                setOk("instagram", true);
                setLoad("instagram", false);
              })
            ...
            setErr(
              "instagram",
              'No Instagram Business account linked to your "' + fb.pageName + '" Facebook Page. Open Instagram app → Edit Profile → tap Facebook → select your page.'
            );
            setLoad("instagram", false);
          }
        }
  ...
  const connectLinkedIn = () => {
    if (!LINKEDIN_CLIENT_ID) {
      setErr(
        "linkedin",
        "LinkedIn Client ID not set. Add VITE_LINKEDIN_CLIENT_ID to your .env file.",
      );
      return;
    }
    const url = new URLSearchParams({
  ...
  const connectTwitter = async () => {
    if (!TWITTER_CLIENT_ID) {
      setErr(
        "twitter",
        "Twitter Client ID not set. Add VITE_TWITTER_CLIENT_ID to your .env file.",
      );
      return;
    }
  ...
  const handleTwitterCallback = useCallback(async (code) => {
    ...
    try {
      ...
      setAccounts((a) => ({
        ...a,
        twitter: { connected: true, username, userId },
      }));
      setOk("twitter", true);
    } catch {
      setErr(
        "twitter",
        "X / Twitter connection failed. Make sure the n8n Twitter OAuth webhook is active.",
      );
    } finally {
      ...
      >
        ...
        >
          ...
          >
            <span
              style={{
                color: connectedCount > 0 ? "#4ade80" : "rgba(255,255,255,0.4)",
              }}
            >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {PLATFORMS.map((p, i) => {
            ...
            const connectedLabel = accounts[p.key]?.pageName
              ...
              <motion.div
                ...
                style={{
                  background: connected
                    ? `linear-gradient(135deg, ${p.color}14, ${p.color}06)`
                    ...
                    {p.oauthType === "platform_whatsapp" ? (
                      ...
                      <button
                        ...
                        style={{
                          ...
                          padding: "10px 18px",
                          background:
                            "linear-gradient(135deg, #ff0050, #ff0050bb)",
                          border: "none",
                          borderRadius: 10,
                          color: "white",
                          fontSize: 14,
                      ...
                      <button
                        ...
                        style={{
                          ...
                          padding: "10px 18px",
                          background:
                            "linear-gradient(135deg, #ea4335, #ea4335bb)",
                          border: "none",
                          borderRadius: 10,
                          color: "white",
                          fontSize: 14,
