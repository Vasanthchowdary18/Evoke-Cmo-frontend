...
import {
  ...
  Check, Loader2, Globe, BookOpen,
  Sparkles, AlertCircle, Activity,
...
const BRAND_VALUE_OPTIONS = [
  'Trust', 'Innovation', 'Quality', 'Sustainability',
  'Creativity', 'Community', 'Excellence', 'Integrity',
  'Transparency', 'Speed', 'Affordability', 'Luxury',
]

const CHANNEL_OPTIONS = [
  { value: 'social_media',    label: 'Social Media',     icon: '📱' },
  { value: 'email',           label: 'Email Marketing',  icon: '📧' },
  { value: 'seo',             label: 'SEO / Blog',       icon: '🔍' },
  { value: 'paid_ads',        label: 'Paid Ads',         icon: '💰' },
  { value: 'whatsapp',        label: 'WhatsApp',         icon: '💬' },
  { value: 'linkedin',        label: 'LinkedIn',         icon: '🤝' },
  { value: 'content',         label: 'Content / Video',  icon: '🎬' },
...
const BUDGET_OPTIONS = [
  ...
  { value: 'no_budget',  label: 'No Paid Budget' },
]
...
function ColorPicker({ value, onChange, label }) {
  ...
  const current  = value || '#c8973e'

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <span style={{ fontSize: 11, fontWeight: 600, color: TEXT2 }}>{label}</span>

      {/* ── Selected colour preview + custom picker trigger ── */}
      <div style={{
        ...
        {/* Large swatch — click to open native colour picker */}
        <button
          ...
          style={{
            ...
            boxShadow: `0 4px 16px ${current}55`,
        ...
        {/* Hex value + hint */}
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: TEXT, letterSpacing: '0.04em' }}>
            {current.toUpperCase()}
          </div>
          <div style={{ fontSize: 11, color: TEXT3, marginTop: 2 }}>
            Click swatch to pick any colour
          </div>
        </div>
      </div>
...
export default function BrandKnowledgeBase() {
  ...
  const [healthScore, setHealthScore] = useState(20) // Brand KB just completed = +20
  ...
  if (showCompletion) {
    ...
    const objectivesLabel = (Array.isArray(form.primaryObjective) ? form.primaryObjective : (form.primaryObjective ? [form.primaryObjective] : [])).map(v => OBJECTIVE_OPTIONS.find(o => o.value === v)?.label).filter(Boolean).join(', ')
    const channelsLabel = (form.primaryChannels || []).map(v => CHANNEL_OPTIONS.find(c => c.value === v)?.label).filter(Boolean).join(', ')
    ...
    // Personalised agent/campaign suggestions — same engine as the Dashboard,
    // scored against the brand profile the user just filled in.
    ...
    const goToRecommendation = (action) => {
      ...
      if (action.path.startsWith('/campaign/')) {
        navigate(action.path, { state: { prefill: buildCampaignPrefill(form) } })
        return
      }
            ...
            <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 18, padding: '20px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 18 }}>
                {[
                  ...
                  { label: 'Tone of Voice', value: form.toneOfVoice },
                  { label: 'Objective', value: objectivesLabel },
                  { label: 'Timeline', value: TIMELINE_OPTIONS.find(t => t.value === form.timeline)?.label },
                  { label: 'Budget', value: BUDGET_OPTIONS.find(b => b.value === form.marketingBudget)?.label },
          ...
          >
            ...
            {step === 2 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Brand Colours</span>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                ...
                {/* Brand Tagline */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Brand Voice</span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: TEXT2 }}>Tone of Voice</span>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      {TONE_OPTIONS.map(t => {
                        const active = form.toneOfVoice === t.value
                        return (
                          <button key={t.value} onClick={() => set('toneOfVoice', t.value)} style={{
                            background: active ? 'rgba(200,151,62,0.12)' : INPUT_BG,
                            border: `1px solid ${active ? GOLD : BORDER}`,
                            borderRadius: 10, padding: '10px 14px', cursor: 'pointer',
                            textAlign: 'left', transition: 'all 0.18s',
                          }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: active ? GOLD : TEXT }}>{t.label}</div>
                ...
                {/* Visual Style */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Visual Style</span>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                    {VISUAL_STYLE_OPTIONS.map(s => {
                      const active = form.visualStyle === s.value
                      return (
                        <button key={s.value} onClick={() => set('visualStyle', s.value)} style={{
                          background: active ? 'rgba(200,151,62,0.12)' : INPUT_BG,
                          border: `1px solid ${active ? GOLD : BORDER}`,
                          ...
                          <span style={{ fontSize: 18, color: active ? GOLD : TEXT2 }}>{s.icon}</span>
                          <span style={{ fontSize: 12, fontWeight: 600, color: active ? GOLD : TEXT }}>{s.label}</span>
                        </button>
                      )
                    })}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Brand Values</span>
                    <span style={{ fontSize: 11, color: TEXT3 }}>optional · select all that apply</span>
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {BRAND_VALUE_OPTIONS.map(v => {
                      const selected = (form.brandValues || []).includes(v)
                      return (
                        <button
                          key={v}
                          onClick={() => {
                            const cur = form.brandValues || []
                            set('brandValues', selected ? cur.filter(x => x !== v) : [...cur, v])
                          }}
                          style={{
                            padding: '7px 14px', borderRadius: 100, cursor: 'pointer',
                            background: selected ? 'rgba(200,151,62,0.14)' : INPUT_BG,
            ...
            {step === 3 && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 26 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Primary Objective</span>
                    <span style={{ fontSize: 11, color: '#ef4444', fontWeight: 600 }}>required</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                    {OBJECTIVE_OPTIONS.map(o => {
                      const active = Array.isArray(form.primaryObjective) ? form.primaryObjective.includes(o.value) : form.primaryObjective === o.value
                      return (
                        <button key={o.value} onClick={() => {
                          const curr = Array.isArray(form.primaryObjective) ? form.primaryObjective : (form.primaryObjective ? [form.primaryObjective] : [])
                          set('primaryObjective', curr.includes(o.value) ? curr.filter(v => v !== o.value) : [...curr, o.value])
                        }} style={{
                          background: active ? 'rgba(200,151,62,0.12)' : INPUT_BG,
                          border: `1px solid ${active ? GOLD : BORDER}`,
                          borderRadius: 10, padding: '10px 14px', cursor: 'pointer',
                          ...
                          <div style={{ fontSize: 13, fontWeight: 600, color: active ? GOLD : TEXT }}>{o.label}</div>
                ...
                {/* Targets */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Targets</span>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <Field label="Target Number / Metric">
                      <select value={form.objectiveTarget} onChange={e => set('objectiveTarget', e.target.value)} style={selectStyle}>
                        <option value="">Select a target…</option>
                        <option value="10% growth">10% growth</option>
                        <option value="20% growth">20% growth</option>
                        <option value="30% growth">30% growth</option>
                        <option value="50% growth">50% growth</option>
                        <option value="100 leads/mo">100 leads / mo</option>
                        <option value="500 leads/mo">500 leads / mo</option>
                        <option value="1,000 leads/mo">1,000 leads / mo</option>
                        <option value="5,000 leads/mo">5,000 leads / mo</option>
                        <option value="10K followers">10K followers</option>
                        <option value="50K followers">50K followers</option>
                        <option value="100K followers">100K followers</option>
                        <option value="2x ROAS">2x ROAS</option>
                        <option value="3x ROAS">3x ROAS</option>
                        <option value="5x ROAS">5x ROAS</option>
                    ...
                    <Field label="Revenue Target">
                      <select value={form.revenueTarget} onChange={e => set('revenueTarget', e.target.value)} style={selectStyle}>
                        <option value="">Select a revenue target…</option>
                ...
                {/* Timeline */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Timeline</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Marketing Budget</span>
                    <span style={{ fontSize: 11, color: TEXT3 }}>helps AI prioritise channels</span>
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {BUDGET_OPTIONS.map(b => {
                      const active = form.marketingBudget === b.value
                      return (
                        <button key={b.value} onClick={() => set('marketingBudget', b.value)} style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: GOLD, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Primary Channels</span>
                    <span style={{ fontSize: 11, color: TEXT3 }}>select all you want to use</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
                    {CHANNEL_OPTIONS.map(ch => {
                      const selected = (form.primaryChannels || []).includes(ch.value)
                      return (
                        <button
                          key={ch.value}
                          onClick={() => {
                            const cur = form.primaryChannels || []
                            set('primaryChannels', selected ? cur.filter(x => x !== ch.value) : [...cur, ch.value])
                          }}
                          style={{
                            padding: '10px 8px', borderRadius: 10, cursor: 'pointer',
                            background: selected ? 'rgba(200,151,62,0.12)' : INPUT_BG,
                            border: `1px solid ${selected ? GOLD : BORDER}`,
                            color: selected ? GOLD : TEXT2,
                ...
                <Field label="Additional Notes" hint="anything your AI CMO should know">
                  <textarea value={form.additionalNotes} onChange={e => set('additionalNotes', e.target.value)}
                    placeholder="Upcoming product launches, seasonal campaigns, markets to avoid, special instructions…" style={taStyle} />
                </Field>

                {/* AI CMO Objective preview */}
                {(Array.isArray(form.primaryObjective) ? form.primaryObjective.length > 0 : !!form.primaryObjective) && form.objectiveTarget && form.timeline && (
                  <div style={{
                    background: 'rgba(200,151,62,0.07)', border: `1px solid rgba(200,151,62,0.28)`,
                    borderRadius: 12, padding: '14px 18px',
                  }}>
                    <div style={{ fontSize: 11, color: GOLD, fontWeight: 700, marginBottom: 6, letterSpacing: '0.06em' }}>
                      YOUR AI CMO OBJECTIVE
                    </div>
                    <div style={{ fontSize: 14, color: TEXT, fontWeight: 600, lineHeight: 1.6 }}>
                      "{(Array.isArray(form.primaryObjective) ? form.primaryObjective : [form.primaryObjective]).map(v => OBJECTIVE_OPTIONS.find(o => o.value === v)?.label).filter(Boolean).join(' + ')} by {form.objectiveTarget}
