...
export default function ConnectAccounts() {
  ...
  const connectFacebook = () => {
    if (!window.FB) {
      setErr(
        "facebook",
        "Facebook SDK not loaded. Please refresh and try again.",
      );
      return;
    }
    ...
    window.FB.login(
      (resp) => {
        if (!resp || !resp.authResponse) {
          ...
          setErr(
            "facebook",
            "Facebook login cancelled or popup blocked. Allow popups and try again.",
          );
          setLoad("facebook", false);
          return;
        }
        ...
        window.FB.api(
          ...
          (pagesResp) => {
            if (!pagesResp || pagesResp.error || !pagesResp.data?.length) {
              ...
              setErr(
                "facebook",
                pagesResp?.error?.message || "No Facebook Pages found.",
              );
              setLoad("facebook", false);
              return;
            }
